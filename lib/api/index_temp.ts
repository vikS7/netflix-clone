...
export const getMovieCredits = async (id: any): Promise<API.Credit> => {
    const results = await db<API.Credit>(`movie/${id}/credits`);
    return results;
}
...
export const searchQuery =  (query: string) => {
    return  db<API.Page<API.Show>>('search/multi', `&query=${query}`);
};
